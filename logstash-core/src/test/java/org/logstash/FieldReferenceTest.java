...
public class FieldReferenceTest {
    ...
    public void testParseSingleBareField() throws Exception {
        assertArrayEquals(FieldReference.parse("foo"), new String[]{"foo"});
    }
    ...
    public void testParse2FieldsPath() throws Exception {
        assertArrayEquals(FieldReference.parse("[foo][bar]"), new String[]{"foo", "bar"});
    }

    @Test
    public void testParse3FieldsPath() throws Exception {
        assertArrayEquals(
            FieldReference.parse("[foo][bar][baz]"), new String[]{"foo", "bar", "baz"}
        );
    }
}
