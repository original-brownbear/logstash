...
import org.logstash.cluster.protocols.raft.storage.snapshot.Snapshottable;
import org.logstash.cluster.time.WallClockTimestamp;

/**
 * Base class for user-provided Raft state machines.
 *
 * Users should extend this class to create a state machine for use within a {@link org.logstash.cluster.protocols.raft.RaftServer}.
 *
 * State machines are responsible for handling {@link org.logstash.cluster.protocols.raft.operation.RaftOperation operations} submitted to the Raft cluster and
 ...
 *
 * When RaftCommand commands and RaftQuery queries
 * (i.e. <em>operations</em>) are submitted to the Raft cluster, the {@link org.logstash.cluster.protocols.raft.RaftServer} will log and replicate them as
 ...
 * by the state machine during setup or can be explicitly registered by overriding the configure(RaftServiceExecutor)
 ...
 * be deterministic. That is, state machines are guaranteed to see RaftCommands in the
 ...
 *   }
 * </pre>
 * For snapshottable state machines, Raft will periodically request a {@link Snapshot Snapshot}
 * of the state machine's state by calling the {@link Snapshottable#snapshot(SnapshotWriter)}
