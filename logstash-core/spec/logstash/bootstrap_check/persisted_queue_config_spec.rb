...
describe LogStash::BootstrapCheck::PersistedQueueConfig do
  ...
  context("when persisted queues are enabled") do
    ...
    context("and 'queue.max_bytes' is set to a value greater than the value of 'queue.page_capacity'") do
      it "should not throw if enough space available" do
        settings.set_value("queue.max_bytes", 1024 ** 2)
        expect { LogStash::BootstrapCheck::PersistedQueueConfig.check(settings) }.to_not raise_error
      end

      it "should throw if not enough space available" do
        settings.set_value("queue.max_bytes", 1024 ** 6)
        expect { LogStash::BootstrapCheck::PersistedQueueConfig.check(settings) }.to raise_error
      end
    end
  end
